...
사람들이 카테고리별 총 긍/부정 평가를 한 개수는 이렇고(막대 그래프)
개수에 따른 긍정과 부정 평가의 비율을 확인하니 (원그래프)
긍정 비율은 `만족도` 카테고리가 제일 높았고 부정 비율은 `가격` 카테고리가 제일 높았다.
이를 통해 사람들은 음식점에 대한 `만족도`는 긍정적이었지만 `가격`에 대한 평가는 부정적이었다.
`만족도` 카테고리의 긍정 리뷰글과 `가격` 카테고리의 부정 리뷰글의 명사와 동사/형용사의 빈출 정도를 확인하면 이렇다 (워드클라우드)
...
하지만, `만족도` 카테고리와 `가격` 카테고리의 명사 워드 클라우드 자료를 확인시, (워드 클라우드) "디저트" 단어의 빈출이 높다는 것을 알 수 있다.
...
두 카테고리에 높은 빈출을 나타내던 "디저트"는 `만족도` 카테고리에선 90% 이상의 긍정, `가격` 카테고리에선 70% 후반대의 긍정 결과가 나왔다.(Pie chart) 또한 마찬가지로 높은 빈출을 나타내던 "음식"은 `만족도` 카테고리에선 90% 이상의 긍정, `가격` 카테고리에선 80% 초반대의 긍정 결과가 나왔다.
마지막으로 `만족도` 카테고리의 "친절"에 대해선 90% 이상의 긍정적 평가 결과가 나왔고, `가격` 카테고리의 "뷔페"에 대해선 60% 후반대의 긍정적인 평가 결과가 나왔다.
이를 종합하면, 더 파크뷰는 대부분 긍정적인 리뷰글로 구성되어 있지만, "가격"에 대해선 부정의 의견이 공통적으로 등장했다.

(키워드중 특정 키워드로 제일 많이 나온 디저트 키워드 개수를 구한 뒤 이를 보여주며)
신라호텔 더 파크뷰는 여러 음식을 즐길 수 있는 뷔페임에도 디저트에 대한 만족도가 많이 큰 것으로 드러났다.