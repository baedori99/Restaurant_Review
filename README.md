...
##### 문제점1 : 리뷰에 음식에 대한 평가에서 긍정과 부정이 한꺼번에 들어갈 수 있음을 인지함.
  - 이를 프롬프트로 고쳐 볼려고 했지만 프롬프트에 많은 일을 시키게 되면서 과부하 비슷한 일 발생
##### 해결방법1: 우리가 이 과제를 하는 이유중 하나는 스스로 작성해보며 포트폴리오를 만들기 위한 기초를 쌓는 중.
...
##### `만족도` 카테고리의 부정글만 모아 명사, 동사/형용사를 추출하여 워드클라우드로 시각화를 헤야하지 않을까?
...
- 이를 통해 `만족도` 카테고리에서 긍정평가를 했던 사람들은 제일 언급된 빈도 수가 많았던 3개의 명사에 대해서는 어떻게 평가했는지 알 수 있을듯 (`가격` 또한 같음)
  ...
  - `가격` 카테고리의 부정 리뷰글에 언급된 빈도 수가 높았던 명사
    - 뷔페 : 긍정의 비율이 60% 후반대를 기록하며 `만족도`카테고리에 비해 긍정의 비율이 떨어지지만 뷔페에 대한 평가는 전반적으로 긍정적이었다고 볼 수 있음.
    - 음식 : 긍정의 비율이 80% 초반대를 기록하며 `만족도`카테고리의 음식 긍정의 비율과 비슷한 긍정의 비율을 보여주며 사람들 대부분이 음식에 대해 긍정적이었다.
...
## 보고서 양식
### 피드백
  - Insight : 데이터 분석을 하지 않았다면 몰랐을 데이터 특징을 찾아내는 것
    - ex) 호텔 뷔페가 "맛있다", "비싸다"라는 것은 데이터 분석을 하지 않아도 알 수 있는 부분
    - 하지만, 만족도가 낮은 리뷰들이 있었다면 주로 어떤 부분에 불만족이었는지는 데이터를 통해서만 알 수 있다.

### 데이터 분석을 통해 알 수 있었던 점들 (Insight)
#### 프로젝트의 결론 짓기
- 만족도가 낮은 리뷰들을 따로 구한다?

##### 데이터에 대한 내 생각
  1. 데이터 전처리를 마치고 긍정/부정 분류를 끝낸 데이터프레임
    - 각 카테고리의 긍/부정의 총 갯수를 구한 결과
      - `만족도`, `맛`, `서비스`, `가격` 카테고리에서 `만족도`, `맛`, `서비스`, `가격`순으로 긍/부정 평가를 많이 했다.
      - 사람들은 4가지 카테고리중에서 `만족도` 항목에서 제일 긍정적으로 평가하였음.
      - 사람들은 4가지 카테고리중에서 `가격` 항목에서 제일 부정적으로 평가하였음.

    - `만족도`, `가격` 카테고리의 각 긍정 리뷰글, 부정 리뷰글의 명사,동사/형용사를 추출하여 시각화한 결과
      - `만족도` 카테고리의 긍정 리뷰글에서 언급된 빈도 수가 높은 명사로는 "음식", "친절", "디저트"가 있었음.
      - `만족도` 카테고리의 긍정 리뷰글에서 언급된 빈도 수가 높은 동사/형용사로는 ("가다","오다","먹다"등의 헹위에 대한 단어 빼고) "맛있다", "비싸다", "아쉽다"가 있었음
      - `가격` 카테고리의 부정 리뷰글에서 언급된 빈도 수가 높은 명사로는 "뷔페","음식","디저트"가 있었음.
      - `가격` 카테고리의 부정 리뷰글에서 언급된 빈도 수가 높은 동사/형용사로는 ("먹다","하다","가다"등의 행위에 대한 단어 빼고) "맛있다", "비싸다", "아쉽다"가 있었음
      - 위의 시각화 자료를 통해 `만족도` 긍정 리뷰글에서 명사 단어들과 동사/형용사 단어들을 같이 보았을 때 `만족도`이외에 음식에 대한 긍정적인 평가와 
        - 명사는 빈도수가 높지 않지만 "비싸다"라는 동사/형용사단어의 빈도 수를 통해 가격에 대한 부정적인 평가가 예상이 된다.
      - 이외에는 단어들에 대해 예상할 수 있는 정보가 적기 때문에 확인이 어렵다.
      - 위의 시각화 자료를 통해 `가격` 부정 리뷰글에서 명사단어들과 동사/형용사단어들을 같이 보았을 때 `가격`이외에 음식에 대한 긍정적인 평가가 이어졌다.
        - 하지만, 이외에 단어들에서 `가격`이외에 예상할 수 있는 정보가 적기 때문에 확인이 어렵다.

  2. `만족도`, `가격` 카테고리의 각 리뷰글의 언급된 빈도 수가 높은 명사 3개의 긍정/부정 분류하여 데이터 프레임에 저장
    - 빈도 수가 높은 명사들의 긍/부정의 총 갯수를 구한 결과
      - `만족도` 카테고리의 긍정 리뷰글의 빈도 수 높은 명사는 `음식`,`친절`,`디저트`순으로 긍/부정 평가를 많이 했다.
      - `가격` 카테고리의 부정 리뷰글의 빈도 수 높은 명사는 `음식`,`뷔페`,`디저트`순으로 긍/부정 평가를 많이 했다.
    - 빈도 수가 높은 명사들의 긍/부정 비율
      - `만족도` 카테고리의 긍정 리뷰글의 빈도 수 높았던 명사들의 긍/부정 비율은
        - `음식`, `친절`,`디저트` 전부 긍정의 비율이 90% 초중반대를 기록하였다.
      - `가격` 카테고리의 부정 리뷰글의 빈도 수 높았던 명사들의 긍/부정 비율은
        - `뷔페`는 긍정 68%를 기록하며 다른 명사들에 비해 수치가 낮았다.
        - `음식`은 긍정 81%대를 기록하였다.
        - `디저트`는 긍정 79%를 기록하였다.
      - `만족도`, `가격` 카테고리 리뷰글들의 빈도 수 높은 명사들은 카테고리의 긍/부정 상관없이 모두 긍정적인 평가를 하였다.

##### 데이터를 분석하며 아쉬운 점, 부족했던 점에 대한 생각 정리
  - 일단 처음엔 좋은 생각같았지만 데이터 분석을 하면 할수록 안좋았다고 생각했던 점
    - 데이터에 대한 긍/부정 분류를 카테고리에 따라 하게 되었을 때,
      - 좀 더 많은 카테고리를 대상으로 분류를 했더라면 더 질 좋은 데이터들이 추출되어 나올수 있었다고 생각한다.
        - `서비스`,`분위기`등
  - 워드 클라우드 자료중 리뷰글의 명사 추출을 한 자료가 있는데 거의 언제나 `음식` 카테고리가 최상위권이었다.
    - 만약 긍/부정 리뷰를 한다고 해도 음식에 대한 긍/부정을 확인한 뒤
    - 긍정적이라면 어떠한 음식들이 사람들에게 긍정적이었고 무슨 음식이 사람들에게 부정적이었는지 확인해 볼 수 있다고 생각한다.
  - 또한 `디저트`도 언급이 많았는데 피드백 내용처럼 어떠한 디저트들이 언급이 많이 되었는지 좀 더 구체적으로 확인해보는 것도 좋다고 생각한다.
  - 형태소 분리 하는 과정에서 동사/형용사 분리를 하고 나니 `하다`,`가다`,`있다`등의 많이 나오지만 행위에 대한 단어로 소위 영양가가 없는 동사/형용사들이 높은 빈도 수를 차지했다.
    - 분석을 하는 입장에서 행위에 대한 단어로 긍/부정을 판단하는데 도움이 되지 않는 동사/형용사들에 대한 불용어 처리를 해야 겠다고 생각한다.
    - 불용어 처리를 하고 추출되는 빈도 높은 동사/형용사들은 좀 더 긍/부정을 판단하는데 도움을 줄 수 있다고 생각하고 결론에 대해 설명할 때 뒷받침이 될만하다고 예상한다.
  - 카테고리를 나눠 긍/부정을 확인하는 방법에서
    - 리뷰글의 경우 리뷰글 안에 카테고리에 해당하는 내용이 여러가지 들어가 있을 수 있고, 또한 하나의 카테고리에 대한 긍정과 부정의 내용이 글 하나에 다들어갈 수 있다.
    - 이러한 경우에 대한 새로운 분류 방법이 필요하다고 생각한다.
      - 생각해볼수 있는 방법
        - 프롬프트로 `{대상: 대상의 긍/부정}`을 쓰면 된다. (피드백때 나온 내용)
        - 프롬프트를 좀 더 구체적으로 한다면 우리는 리뷰글에서 그 글의 카테고리가 긍/부정을 판별하는 것이다.
          - 그러므로 프롬프트로 리뷰글의 그 카테고리의 긍/부정의 갯수를 파악한 뒤 갯수가 많은 쪽으로 분류를 하는 방법도 생각해볼 수 있다.
          - 혹은 긍/부정 갯수를 출력하게 하는 방법도 있다.
  - 카테고리중 가장 많은 긍/부정 평가 갯수를 가진 카테고리를 평가 갯수가 많은 쪽만 분석을 할게 아닌 그 반대쪽도 분석을 했어야 됬다.
    - 예를 들어 `만족도`는 4가지의 카테고리중 제일 많은 긍/부정 갯수를 가지고 있다.
    - 하지만 그 중 긍정 평가에 대한 갯수가 월등히 많았다.
    - 그렇다면 그 중 부정 평가는 되게 소수인데 어떤 요소가 만족도 부분에서 부정적으로 평가되었는지 확인을 해야했다고 생각한다.
    - 아니면 부정의 갯수가 제일 많은 카테고리에서 어떤 요소가 긍정평가를 이끌어냈는지도 분석을 해봤어야했다.
  - 명사, 동사/형용사의 빈도 수가 높은 단어들로만 선정? or 영향력이 높은 단어를 선별? 등 단어 선택 방법
    - 골라야 할 순서를 정해두는 것이 좋을 거 같다.
      - 워드 클라우드를 통해 제일 큰 단어를 선택을 한다.
      - if 그 단어가 카테고리들과 연관되어 있거나 or 선정한 카테고리이외에서 나오는 경우
      - "음식"과 "디저트"는 작성자의 솽황에 따라 같은 것으로 치부한다.
      - (피드백에서 나온 내용) 빈도 수가 많다는 건 일반적인 단어라 많이 쓰였을 가능성이 크다.
        - 호텔 뷔페에서 단어 중 뷔페, 호텔과 같은 단어는 많이 나왔을수밖에 없다.
        - 얘상 외의 단어들에 좀 더 집중을 해보는 것도 좋았을거 같다.
  - 나중에는 좀 더 많은 리뷰의 수를 크롤링하여 분석을 해보고 싶다.
    - 처음에 프로젝트를 시작했을 때에는 200개의 리뷰글이면 어느정도 윤곽이 많이 잡힐 수 있다고 생각했다.
    - 하지만 데이터 전처리 과정을 지나며 리뷰글의 갯수가 적다는 것을 분석을 하며 느끼게 되었다.
    - 리뷰글의 갯수에 대한 결정을 어떤 기준으로 정해야할지 감이 잡히지 않는다.
---
